...
extension Requestable {
    ...
    public func url(with config: NetworkConfigurable) throws -> URL {
        ...
        var baseURL = config.baseURL.absoluteString.last != "/" ? config.baseURL.absoluteString + "/" : config.baseURL.absoluteString
   			...
   			baseURL.insert(contentsOf: domain + ".", at: index)
        ...
        queryParameters.forEach {
			if let array = $0.value as? [String] {
				for value in array {
    ...
    public func urlRequest(with config: NetworkConfigurable) throws -> URLRequest {
        ...
        } else if bodyParametersEncodable != nil {
            urlRequest.httpBody = try? bodyParametersEncodable?.toData()
        }
