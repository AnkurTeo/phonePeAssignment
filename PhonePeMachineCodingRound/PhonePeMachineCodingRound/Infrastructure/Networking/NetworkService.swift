...
public enum NetworkError: Error {
    ...
    case cancelled
    case generic(Error)
    case customeError(message: String)
    case urlGeneration
}
...
public protocol NetworkService {
    typealias CompletionHandler = (Result<Data?, NetworkError>, Int?, URLRequest?) -> Void
    ...
    private func request(request: URLRequest, completion: @escaping CompletionHandler) -> NetworkCancellable {
        ...
        let sessionDataTask = sessionManager.request(request) { data, response, requestError in
            if let requestError = requestError {
                ...
                completion(.failure(error), nil, request)
            } else {
							  let resp = response as! HTTPURLResponse
							  let status = resp.statusCode
                self.logger.log(responseData: data, response: response)
                completion(.success(data), status, request)
            }
        }
...
extension DefaultNetworkService: NetworkService {
    ...
    public func request(endpoint: Requestable, completion: @escaping CompletionHandler) -> NetworkCancellable? {
        do {
            ...
            completion(.failure(.urlGeneration), nil, nil)
            return nil
        }
    }
}
